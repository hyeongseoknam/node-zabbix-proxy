...
class ZabbixClientSimulator(protocol.Factory):
    ...
    def do(self):
        ...
        if self.__active_checks:
            print 'def do(self): step -4'
            self.__sendData()

    def __sendData(self):
        print 'def __sendData(self): step -1'
        self.__sendbuf.append(ZabbixHelper().getAgentData(self.__hostname, self.__active_checks))
...
class ZabbixSender(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        if self.__parseHeader(data):
            print 'parse complete:', self.__doc
...
class ZabbixHelper(object):
    ...
    def getAgentData(self,host= None, active_checks= {}):
        ...
        for key, item in active_checks.items():
            value = int(random.randrange(1,100))
            item_values.append(dict(host = host, key= key, value=value, clock= now))
